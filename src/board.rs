...
use crate::types::{Color, Piece, Square, Move, UnmakeInfo, TranspositionTable, BoundType, MATE_SCORE, file_to_index, rank_to_index};
use crate::utils::{mvv_lva_score, format_square};
...
impl Board {
    ...
    pub fn generate_knight_moves(&self, from: Square) -> Vec<Move> {
        let mut moves = Vec::new();
        let deltas = [
            (2, 1),
            (1, 2),
    ...
    pub fn generate_sliding_moves(&self, from: Square, directions: &[(isize, isize)]) -> Vec<Move> {
        let mut moves = Vec::new();
        let (rank, file) = (from.0 as isize, from.1 as isize);
        let color = self.current_color();
    ...
    pub fn generate_king_moves(&self, from: Square) -> Vec<Move> {
        let mut moves = Vec::new();
        let deltas = [
            (1, 0),
            (-1, 0),
    ...
    pub fn generate_pawn_tactical_moves(&self, from: Square, moves: &mut Vec<Move>) {
        let color = self.current_color();
        let dir: isize = if color == Color::White { 1 } else { -1 };
        let promotion_rank = if color == Color::White { 7 } else { 0 };
