...
#[inline]
fn sq_to_bb(sq: Square) -> BitBoard { 1u64 << (sq.0 * 8 + sq.1) }
...
#[inline]
fn bishop_attacks_from(sq: Square, occ: BitBoard) -> BitBoard {
    const DIAGS: &[(isize, isize)] = &[(1, 1), (1, -1), (-1, 1), (-1, -1)];
    ray_attacks(sq, occ, DIAGS)
}

#[inline]
fn rook_attacks_from(sq: Square, occ: BitBoard) -> BitBoard {
