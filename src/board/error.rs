...
pub enum FenError {
    ...
    InvalidEmptySquareCount { char: char },
...
impl fmt::Display for FenError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            FenError::InvalidEmptySquareCount { char } => {
                write!(f, "Invalid empty-square count '{char}' in FEN")
            }
            ...
            FenError::TooFewRanks { found } => {
                write!(f, "Too few ranks ({found}) in FEN")
            }
