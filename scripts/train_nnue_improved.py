...
class NNUE256(nn.Module):
    ...
    def load_quantized(self, path: str):
        ...
        self._load_quantized(path, expand_piece_square=False)

    def load_piece_square_quantized(self, path: str):
        """Load a 768-input network, zeroing any additional tactical rows."""
        self._load_quantized(path, expand_piece_square=True)

    def _load_quantized(self, path: str, expand_piece_square: bool):
        ...
        allowed_inputs = (BASE_INPUT_SIZE, INPUT_SIZE) if expand_piece_square else (INPUT_SIZE,)
        if input_size not in allowed_inputs or hidden_size != HIDDEN_SIZE:
            raise ValueError(
                f"NNUE architecture mismatch: file is {input_size}->{hidden_size}, "
                f"trainer expects inputs {allowed_inputs} and {HIDDEN_SIZE} hidden units"
            )
        weight_count = input_size * HIDDEN_SIZE
        expected_bytes = offset + (weight_count + 3 * HIDDEN_SIZE + 1) * 2
        if len(data) != expected_bytes:
            raise ValueError(f"NNUE file has {len(data)} bytes; expected {expected_bytes}")

        # Validate and decode the entire file before changing any parameters.
        values = torch.frombuffer(bytearray(data[offset:]), dtype=torch.int16).to(torch.float32)
        weights = values[:weight_count].reshape(input_size, HIDDEN_SIZE)
        bias, us_weights, them_weights = values[weight_count:-1].reshape(3, HIDDEN_SIZE)
        with torch.no_grad():
            self.feature_weights.zero_()
            self.feature_weights[:input_size].copy_(weights / QA)
            self.feature_bias.copy_(bias / QA)
            self.output_weights_white.copy_(us_weights / QB)
            self.output_weights_black.copy_(them_weights / QB)
            self.output_bias.copy_(values[-1:] / (QA * QB))
